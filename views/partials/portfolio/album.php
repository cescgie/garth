      ...
      <div class="col s12 m4 l3 foto_album">
        <div class="card">
          <div class="card-image waves-effect waves-block waves-light album-image">
            <a class="group4"  title="<?= str_replace('_',' ',$value['title'])?>" href="<?= DIR.$value['path'];?>">
              <?php if($value['bild_form']==1) :?>
              <img style="border-color:white;width:60%;position:absolute;margin:auto" class="responsive-img hoverable z-depth-3 activator" src="<?= DIR.$value['cover'];?>">
            <?php else:?>
          ...
          <div class="card-content">
            <!-- <a href="<?php DIR ?>show?album_id=<?= $value['album_id'];?>&kategorie_id=<?= $value['kategorie_id'];?>&reihenfolge=<?= $value['reihenfolge'];?>">
              <p class="center activator grey-text text-darken-4 "><?= str_replace('_',' ',$value['title']); ?></p>
            </a> -->
            <?php if(SESSION::get('admin')) :?>
              ...
              <div id="modalEdit<?= $value['id'];?>" class="modal modal-fixed-footer">
                <div class="modal-content">
                  <h4>Edit <?= str_replace('_',' ',$value['title']);?></h4>
                  <form id="form<?= $value['id'];?>">
                    <div class="row">
                      <div class="input-field col s12">
                        <input value="<?= str_replace('_',' ',$value['title']);?>" id="title-<?= $value['id'];?>" type="text" class="validate">
              ...
              <div id="modalDelete<?= $value['id'];?>" class="modal">
                <div class="modal-content">
                  <p>Delete <?= str_replace('_',' ',$value['title']);?>?</p>
                </div>
                <div class="modal-footer">
              ...
              <?php
                ...
                foreach($data['images']as $key=>$value): ?>
                  <li class="ui-state-default">
                    ...
                    <span class="ui-icon ui-icon-arrowthick-2-n-s"></span><span id="title" style="font-size:0.75em;"><?=$i?> <?= str_replace('_',' ',$value['title']);?></span>
  ...
  $(document).on('click','#speichernRH',function(){
    ...
    for (var i = 0; i < item.length; i++) {
      $.ajax({
        ...
        data: { id : item2[i], reihenfolge : i+1,table:'images'},
        dataType: "html",
